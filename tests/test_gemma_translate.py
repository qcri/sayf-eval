...
def _sample():
    ...
    return Sample(index=0, prompt=prompt, target="C",
                  choices=[f"{k}: {v}" for k, v in _EN_CH.items()], metadata={"task_type": "mcq"})
...
def _store(tmp_path):
    ...
    p.write_text(json.dumps({
        "question": _EN_Q, "answers": _EN_CH, "solution": "C",
        "translated_question": _AR_Q, "translated_choices": _AR_CH,
    }, ensure_ascii=False) + "\n", encoding="utf-8")
...
def test_store_miss_falls_back_to_english_then_live(tmp_path):
    miss = Sample(index=1, prompt="Question: brand new?\nOptions: A) x, B) y\n\nq",
                  target="A", choices=["A: x", "B: y"], metadata={"task_type": "mcq"})
    # no live -> identity (English kept), counted as a miss
    t = GemmaArabicTranslator("harness", store=_store(tmp_path))
    assert t.translate_samples(_task(), [miss])[0].prompt == miss.prompt
    assert t._misses == 1
    # with live -> used
    t2 = GemmaArabicTranslator("harness", store=_store(tmp_path),
                               live=lambda q, ch: ("ARQ", {k: "ع" + v for k, v in ch.items()}))
